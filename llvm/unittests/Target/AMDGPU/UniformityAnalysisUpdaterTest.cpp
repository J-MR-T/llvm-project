...
// TODO Less code duplication between these tests, by creating an actual test class to unify some of the before/after test functionality.

// TODO Also should reduce duplication between these tests and the regression tests, some of these are copied and/or adjusted from there.
...
template<typename T>
constexpr auto divergenceStr(T& Info, Instruction* Inst){
...
bool analysesMatch(T& ToTest, R& Oracle, Function* F){
  for(auto& Inst: instructions(F))
    if(ToTest.isDivergent(&Inst) != Oracle.isDivergent(&Inst)){
      dbgs() << "Value " << Inst << " doesn't match, updated analysis says: " << divergenceStr(ToTest, &Inst) << "; fresh analysis says: " << divergenceStr(Oracle, &Inst) << "\n";
      return false;
    }
...
bool matchesAFreshAnalysis(UniformityAnalysisUpdater& Updater, TargetTransformInfo* TTI, Function* F){
...
#define EXPECT_UNIFORM(Inst)\
      EXPECT_FALSE(Updater.isDivergent((Inst)));

#define EXPECT_DIVERGENT(Inst)\
      EXPECT_TRUE(Updater.isDivergent((Inst)));
...
TEST(UniformityAnalysisUpdater, SyntheticInsert1){
  ...
  auto TM = createAMDGPUTargetMachine("amdgcn-amd-", "gfx1010", "+wavefrontsize32");
  ...
  // sanity check with an ASSERT, not because we couldnt continue, but if this is already wrong, the rest of the test will be utterly wrong
  ...
  auto* DivergentAdd1 = dyn_cast<Instruction>(IRB.CreateAdd(&AddrSp5Load, &ThreadId));
  EXPECT_DIVERGENT(DivergentAdd1);

  auto* DivergentAdd2 = dyn_cast<Instruction>(IRB.CreateAdd(&AddrSp5Load, IRB.getInt32(42)));
  EXPECT_DIVERGENT(DivergentAdd2);

  auto* DivergentAdd3 = dyn_cast<Instruction>(IRB.CreateAdd(IRB.getInt32(42), &ThreadId));
  EXPECT_DIVERGENT(DivergentAdd3);

  auto* UniformAdd = dyn_cast<Instruction>(IRB.CreateAdd(IRB.getInt32(42), &AddrSp1Load));
  EXPECT_UNIFORM(UniformAdd);

  auto* StrungTogetherDivergentAdd = dyn_cast<Instruction>(
      IRB.CreateAdd(
        IRB.getInt32(42),
        IRB.CreateAdd(
          IRB.CreateAdd(
  ...
  // delete them again
  for(auto& Inst:{StrungTogetherDivergentAdd, DivergentAdd1,DivergentAdd2, DivergentAdd3, UniformAdd}){
...
TEST(UniformityAnalysisUpdater, SyntheticReplace1){
  ...
  auto& TransitivelyUniform = *BB1->getTerminator()->getPrevNode()->getPrevNode()->getPrevNode();
  auto& AddrSp5Load = *BB0->getTerminator()->getPrevNode();
  auto& ThreadId = *BB0->getFirstNonPHIIt();
  ...
  auto* DivergentAdd1 = dyn_cast<Instruction>(IRB.CreateAdd(&AddrSp5Load, &ThreadId));
  auto* DivergentAdd2 = dyn_cast<Instruction>(IRB.CreateAdd(DivergentAdd1, IRB.getInt32(42)));
  auto* DivergentAdd3 = dyn_cast<Instruction>(IRB.CreateAdd(IRB.getInt32(42), DivergentAdd2));
  EXPECT_DIVERGENT(DivergentAdd3);
...
TEST(UniformityAnalysisUpdater, SyntheticReplaceAllUniformWithLotsOfDivergence){
  StringRef FuncName = "hidden_loop_diverge";

  // NOTE: in the test function, the CHECK checks indicate what should happen when the %tid is actually divergent,
...
; CHECK: DIVERGENT: %div.merge.exit =
  ...
  auto [_1, _2, UnifWithoutTidOriginal] = initiallyComputeUniformityAnalysis(&TTI, FWithoutTidOriginal);

  // assert that the initial function has the expected uniformity, otherwise the test is broken and the results will only be confusing
  for(auto& Inst: instructions(FWithoutTidOriginal))
      ASSERT_TRUE(UnifWithoutTidOriginal.isUniform(&Inst)) << "Tested function is wrong somehow - should be entirely uniform";
  ...
  // we want to make sure the updater properly propagates a change in divergence from the start of the function to the very end
...
; CHECK: DIVERGENT:  %div.user =
  ...
  // now try the same, but inserting a new cycle (that is in itself uniform), and a termporally divergent use outside
  ...
  // important: don't EXPECT_UNIFORM here! otherwise the traversal later on will behave differently!

  IRB.SetInsertPoint(divUser);
  auto* temporallyDivergentUseOfNewValue = IRB.CreateAdd(uniIncAgain, IRB.getInt32(5));
...
define void @asm_mixed_sgpr_vgpr(i32 %divergent) {
  ...
  ASSERT_TRUE(ModuleString.contains(FuncName)) << "Forgot to update function name?";
  ...
  // DONT actually delete (`eraseFromParent()`) it! we basically want to simulate what happens when a new instruction gets allocated to this spot
  sgpr->removeFromParent();
  Updater.informAboutDeletion(sgpr);

  // -> simulate this, by just editing the sgpr so that its basically the same instruction as the vgpr. The vgpr should definitely be divergent, but if sgpr was not removed from both KnownValues and UniformOverrides, it will be marked uniform by the updater
  // TODO Think about whether to leave it like this. This *should* technically be safe, because the underlying thing is just a vector that doesn't hold const values, and only the way the pointer is returned means its const, but it's still not nice.
  *const_cast<unsigned*>(sgpr->indices().begin()) = 1;

  // essentially treat sgpr as a newly inserted instruction (also need to actually insert it again)
...
loopjoin:
  ...
  ASSERT_TRUE(ModuleString.contains((" @" + FuncName).str())) << "Forgot to update function name?";
...
; the way this is right now, this won't be temporally divergent, but once the H exit branch is divergent, it will be
  ...
  UniformityAnalysisUpdater Updater(Unif);
  // now make the loop exit branch divergent, generating temporal divergence at soonDivUser
  // -> if the sync dependence analysis (SDA) was not up to date, then the replace on the terminator condition would still trigger an `analyzeControlFlowDivergence` call, but the `getJoinBlocks` call inside there would go to the CachedControlDivDescs cache, and wrongly return no join blocks, so no temporal divergence will be propagated. If the SDA works correctly, the join block will be returned and temporal divergence will be propagated, so `soonDivUser` will be marked divergent
...
TEST(UniformityAnalysisUpdater, Fuzzer){
  // TODO Implement the fuzzer, although maybe not in this file but in a separate one.
}



TEST(UniformityAnalysisUpdater, GenericRegressionTestComparison){
  // idea of this test: go through the regression tests for unif analysis, and replace every value with itself, essentially erasing uniforimty info, but keeping control flow in tact
  // -> then check that the resulting analysis matches

  unsigned numFuncs = 0;

  // TODO how do I do this so it works independently of which working dir the tests were started with?
  ...
  for(auto testFile:{
    ...
    LLVMContext Context;
    SMDiagnostic Err;
    std::unique_ptr<Module> M = llvm::parseIRFile(std::string(testFilePrefix)+std::string(testFile), Err, Context);
    ASSERT_TRUE(M) << "Bad LLVM IR as test input? Error: " << Err.getMessage() << "\n---\nLine/Col: " << Err.getLineNo() << "/" << Err.getColumnNo();
    ASSERT_FALSE(verifyModule(*M)) << "Module verification failed for file " << testFile;
    auto TM = createAMDGPUTargetMachine("amdgcn-amd-", "gfx1010", "+wavefrontsize32");
    ...
    for(Function& FRef: M->functions()){
      ...
      // go through all the values except terminators, and replace them with clones of themselves
      for(auto& Inst: llvm::make_early_inc_range(instructions(*F))){
        if(Inst.isTerminator())
          continue;

        auto [_3, _4, UnifFreshBefore] = initiallyComputeUniformityAnalysis(&TTI, F);
...
#define PRINT_DEBUG_POSITION "file " << testFile << " function " << F->getName() << " replacing " << *Clone << " with itself"
